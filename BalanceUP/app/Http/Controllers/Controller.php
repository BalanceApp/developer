...
class Controller extends BaseController
{
    ...
    public function createTable(){
        ...
        DB::statement('CREATE TABLE IF NOT EXISTS everyday(id int primary key NOT NULL AUTO_INCREMENT, userid varchar(255), height int, weight float, fat float, muscle float, date date)');
        DB::statement('CREATE TABLE IF NOT EXISTS regular(id int primary key NOT NULL AUTO_INCREMENT, userid varchar(255), height int, weight float, fat float, muscle float, frequency int, time varchar(12), regDate date)');
        DB::statement('CREATE TABLE IF NOT EXISTS diet(id int primary key NOT NULL AUTO_INCREMENT, userid varchar(255), stapleFood float, mainDish float, sideDish float, meat float, seafood float, eggs float, beans float, LCvegetables float, GYvegetables float, mushrooms float, seaweeds float, potatoes float, milk float, fruit float, sweets float, saltSweets float, juice float, friedFood float, fastFood float, misoSoup float, MenSoup float, supply float, regDate date)');
